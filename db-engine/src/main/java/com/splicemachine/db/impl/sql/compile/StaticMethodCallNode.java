...
public class StaticMethodCallNode extends MethodCallNode {
		...
		if (javaClassName == null)
		{
			...
			if (ad == null)
			{
				throw StandardException.newException(
                        SQLState.LANG_NO_SUCH_METHOD_ALIAS, alternatives);
			}
	...
	private void resolveRoutine(FromList fromList,
		...
		for (int i = list.size() - 1; i >= 0; i--)
		{
			...
			if( resolveParameters(fromList, subqueryList, aggregateVector, sd, proc) == true)
				break;
		}
		...
		if( ad == null )
		{
			...
			for(int j=0; j<methodParms.length; j++) {
				sb.append(j > 0 ? ", " : "");
				sb.append(methodParms[j].getJSQLType().getSQLType().getTypeName());
			}
			sb.append(") not known, please try\n");
	...
	private boolean resolveParameters(FromList fromList, SubqueryList subqueryList, List<AggregateNode> aggregateVector,
								   SchemaDescriptor sd, AliasDescriptor proc) throws StandardException {
		RoutineAliasInfo routineInfo = (RoutineAliasInfo) proc.getAliasInfo();
		int parameterCount = routineInfo.getParameterCount();
	...
	private void resolvePythonFunction() throws StandardException {
